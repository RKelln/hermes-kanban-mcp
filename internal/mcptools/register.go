...
func Register(srv *mcp.Server, s *Server) {
	...
	addTool(srv, "ticket_request_review", "Request a FINAL REVIEW of a finished ticket: ready/running -> 'review' via the hermes CLI (not a block). The dispatcher then claims the ticket as a REVIEW run and spawns the sdlc-review reviewer; APPROVE completes it to done, changes come back to you. summary is required — it is the reviewer's handoff (what changed, the refs to verify, what you already proved). reviewer resolves as explicit reviewer > MCP_REVIEWER_PROFILE, and is VERIFIED against the installed profile roster — refused if it is not a profile the dispatcher can spawn (a non-profile reviewer parks the card in 'review' forever). When neither source supplies one the flag is omitted and the kernel keeps its own choice: the reviewer recorded by a previous request-changes round (which OVERRIDES the current assignee), else the ticket's existing assignee — that value is verified the same way. Moving a RUNNING ticket additionally requires force: true — it releases your live claim, so pass it only for work you claimed yourself. Push your branch FIRST, and do NOT merge before the verdict: done means merge. (id, board and summary required)",
		requestReviewSchema(),
