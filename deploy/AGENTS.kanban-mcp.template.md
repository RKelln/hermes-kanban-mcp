...
2. **Read** — `ticket_get` for full detail on a ticket you'll touch. **Pass `detail: "full"` whenever you must read a comment verbatim** (a steer, a revision comment, a review verdict): the default `partial` mode clips every comment body at a size cap derived from our own ticket corpus (~1500 runes; half of all comments are longer than the old 500-rune cap, which is how a 2,445-rune steer once arrived as 488 runes), and the clipped tail is usually the payload (the design decision, the requested change, the commit ref). Keep `partial` for scanning and for reading the ticket's own body/metadata.
