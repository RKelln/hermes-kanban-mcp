...
6. **Wait for the review** — long-poll `ticket_events` (pass the last seen event id; `kanban_help` has the default and maximum wait; returns instantly with `ticket_status` if the ticket already left `blocked`) or `ticket_get` until it leaves `blocked`: `done` → merge your branch to main (the reviewer never merges); `ready` → REQUEST CHANGES → re-claim, fix, re-complete; still `blocked` → ESCALATED → surface to the human; do not re-loop.
