...
- **Wait for the review** — completing review-gated work leaves the ticket `blocked`. Poll
  `ticket_events` with a long timeout (`timeout_seconds: 120`) and the last seen event id,
  or `ticket_get`, until it leaves `blocked`: `done` → the review passed → merge your branch to
